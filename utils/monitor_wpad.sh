...
check4() {
    ...
    if ! curl -4 -q -kIsS "$WPAD_URL" | grep ^HTTP | grep -q -w 200; then
    ...
    if ! curl -q -4 -x $P4 -kIsS $CHECK_URL 2>/dev/null | grep -q "^HTTP"; then
        fail=${fail}"Using $P4 to access IPv4 is FAILED."
    else
        echo "Proxy $P4 is OK."
    fi
    echo
...
check6() {
    ...
    if ! curl -q -6 -x $P6 -kIsS $CHECK_URL 2>/dev/null | grep -q "^HTTP"; then
        fail=${fail}"\r\nUsing $P6 to access IPv6 is FAILED."
    else
        echo "Proxy $P6 is OK."
    fi
    echo
