...
if [ -n "$PROXY" ]; then
	...
	if ! curl ${PROXY} ${CURL_TIME} -kIsS https://www.google.com/ >/dev/null; then
...
time grep -v "0.0.0.0" "$ZONE_TMP_FILE" | sed -e 's/\."/"/g' | grep -E -v "$exclude_domain" | tr '[A-Z]' '[a-z]' | $SORT -u >>"$ZONE_FILE"
rm "$ZONE_TMP_FILE"
...
include: "$ZONE_FILE"
EOF
if ! /usr/sbin/unbound-checkconf /tmp/check.conf; then
